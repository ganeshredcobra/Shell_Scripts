...
PASSWD="renjith88"
DEV1="5453e3b0-08ef-11e8-9141-1d8d2edf4f93"
	...
	if [ "$#" -ne 3 ]; then
		...
		if [ $1 == "DEV1" ]; then
			...
			if [ $? -eq 0 ]; then
			 # do something
			 #echo "Return zero $2"
			 SDATE=$(($(date -d "$2 + 1 days" +%s%N)/1000000))
			 echo "SDATE is $SDATE"
			 date -d "$3" > /dev/null 2>&1
			 if [ $? -eq 0 ]; then
				#echo "Return zero $3"
				EDATE=$(($(date -d "$3 + 1 days" +%s%N)/1000000))
				echo "EDATE is $EDATE"
				...
				do
					#Made curl silent
					curl -s -X GET "https://demo.thingsboard.io/api/plugins/telemetry/DEVICE/$DEV1/values/timeseries?keys=${KEYS[0]},${KEYS[1]}&startTs=$SDATE&endTs=$EDATE&interval=6000" --header "Content-Type:application/json" --header "X-Authorization: Bearer $JWT_TOKEN"|jq -r ".${KEYS[i]}|.[]|[(.ts|tostring),(.value|tostring)]| @csv" > ${KEYS[i]}.csv
